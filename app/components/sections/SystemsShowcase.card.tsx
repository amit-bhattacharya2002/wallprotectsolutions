  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-x-0 bottom-0 z-10 px-6 pb-6 pt-20 transition duration-300 ease-out group-hover:translate-y-4 group-hover:opacity-0 group-focus-within:translate-y-4 group-focus-within:opacity-0 group-data-[open=true]:translate-y-4 group-data-[open=true]:opacity-0 motion-reduce:hidden">
        <p className="text-[0.6875rem] font-semibold uppercase tracking-[0.22em] text-white/72">{sys.number}</p>
        <h3 className="mt-2 text-balance text-[1.85rem] font-semibold leading-[1.12] tracking-tight text-white drop-shadow-[0_2px_16px_rgba(7,22,44,0.45)] lg:text-[2.15rem]">
          {sys.name}
        </h3>
        <p className="mt-3 hidden text-sm text-white/70 [@media(hover:none)]:block">Tap for details</p>
      </div>
      ...
      <div className="absolute inset-0 z-20 flex translate-y-full flex-col justify-start overflow-y-auto bg-white/32 px-6 pt-8 pb-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.45)] ring-1 ring-inset ring-white/25 backdrop-blur-[18px] backdrop-saturate-150 transition-transform duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] group-hover:translate-y-0 group-focus-within:translate-y-0 group-data-[open=true]:translate-y-0 motion-reduce:translate-y-0 md:justify-end md:py-7 lg:px-7 lg:py-8">
        <div className="pointer-events-none absolute inset-0 bg-linear-to-t from-[#07162c]/70 via-[#07162c]/28 to-[#07162c]/10" />
        <div className="relative [text-shadow:0_1px_1px_rgba(7,22,44,0.55),0_8px_24px_rgba(7,22,44,0.45)]">
          <p className="text-[0.625rem] font-semibold uppercase tracking-[0.22em] text-[#9BCB4A] md:text-[0.6875rem]">{sys.number}</p>
          <p className="mt-1.5 text-balance text-[1.45rem] font-semibold leading-[1.15] tracking-tight text-white md:mt-2 md:text-[1.85rem] lg:text-[2.25rem]" aria-hidden>
            {sys.name}
          </p>
          <p className="mt-2 text-[0.8125rem] italic leading-snug text-white md:mt-3 md:text-sm md:leading-relaxed">{sys.manufacturer}</p>
          <p className="mt-2.5 text-[0.8125rem] font-medium leading-snug text-white md:mt-4 md:text-sm md:leading-relaxed lg:text-[0.9375rem] lg:leading-[1.65]">
            {sys.description}
          </p>
          <Link
            href={sys.href}
            className="mt-3.5 inline-flex w-fit items-center gap-1.5 text-sm font-semibold text-[#9BCB4A] transition-colors hover:text-white md:mt-5"
          >
            Learn more
