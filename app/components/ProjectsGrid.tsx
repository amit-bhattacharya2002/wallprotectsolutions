...
function ProjectBadges({ project }: { project: Project }) {
  return (
    <div className="flex flex-wrap gap-2">
      <span className="px-2.5 py-1 bg-[#005EB8]/80 backdrop-blur-sm text-white text-xs font-medium rounded-full">
        {project.category}
      </span>
      {project.signature && (
        <span className="px-2.5 py-1 bg-[#005EB8] text-white text-xs font-semibold rounded-full">
          Signature Project
        </span>
      )}
    </div>
...
function GridProjectCard({ project }: { project: Project }) {
  return (
    <Link
      ...
      className="group surface-card flex h-full flex-col overflow-hidden bg-white hover:border-[#64A70B] transition-colors"
    >
      ...
      <div className="flex flex-1 flex-col p-5">
        <h3 className="text-base font-semibold text-[#0f172a] tracking-tight mb-1 group-hover:text-[#64A70B] transition-colors">
          {project.title}
        </h3>
...
function ListProjectCard({ project }: { project: Project }) {
  return (
    <Link
      href={`/projects/${project.slug}`}
      className="group surface-card flex overflow-hidden bg-white transition-colors hover:border-[#64A70B]"
    >
      ...
      <div className="flex min-w-0 flex-1 flex-col justify-center gap-2 p-4 sm:p-5 md:p-6">
        ...
        <h3 className="text-base font-semibold text-[#0f172a] tracking-tight transition-colors group-hover:text-[#64A70B] sm:text-lg">
          {project.title}
        </h3>
        {project.details.location && (
      ...
      <div className="hidden shrink-0 items-center pr-5 text-[#64A70B] opacity-0 transition-all group-hover:opacity-100 sm:flex">
